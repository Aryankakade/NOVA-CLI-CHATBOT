...
async function sendMessage() {
  ...
  try {
    ...
    addMessageToChat(
      "bot",
      `❌ Connection Error: ${err.message}. Please ensure the NOVA backend is running on ${API_BASE}`,
    )
...
async function processFileUpload() {
  ...
  try {
    ...
    if (data.success && data.file && data.analysis) {
      ...
      saveChatToHistory(`[File: ${fileInfo.filename}] ${displayMessage}`, analysis.response)
      ...
      const errorMessage = data.message || "File processing failed"
...
function updateVoiceButtonState() {
  ...
  if (voiceBtn) {
    voiceBtn.innerHTML = isRecording ? '<i class="fas fa-microphone-slash"></i>' : '<i class="fas fa-microphone"></i>'
    ...
    if (response.ok) {
      ...
      if (data.success) {
        ...
        if (messagesContainer) {
          ...
          addMessageToChat(
            "bot",
            `Hello! I'm ${agentInfo.name}. My context has been cleared. How can I help you with a fresh start?`,
          )
        }
...
function handleProfileSubmit(e) {
  ...
  const age = Number.parseInt(ageInput.value.trim())
...
function setupEventListeners() {
  ...
  if (continueToChatBtn) {
    continueToChatBtn.addEventListener("click", (e) => {
      e.preventDefault()
      e.stopPropagation()
      continueToChat()
      return false
  ...
  if (analyzeRepoBtn) {
    analyzeRepoBtn.addEventListener("click", (e) => {
      e.preventDefault()
      e.stopPropagation()
      analyzeGitHubRepo()
      return false
...
function formatMessage(message) {
  ...
  return message
    ...
    .replace(
      /```([\s\S]*?)```/g,
      '<pre class="bg-gray-800 p-3 rounded-lg mt-2 overflow-x-auto"><code class="text-cyan-400">$1</code></pre>',
    )
    .replace(/\n/g, "<br>")
}
...
async function initializeSystemCheck() {
  ...
  try {
    ...
    const [systemInfo, health, status] = await Promise.all([getSystemInfo(), healthCheck(), getSystemStatus()])
